...
export function useOAuthReconnect() {
  ...
  const reconnect = useCallback(async (accountId: string, toolkit: string) => {
    ...
    try {
      ...
      // A Shopify grant is saved under the store Shopify authorized, which may
      // not be this account's. Reporting that as a reconnect would close the
      // card while this account stays lapsed.
