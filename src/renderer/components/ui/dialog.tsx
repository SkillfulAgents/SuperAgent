...
>(({ className, children, onKeyDown, hideClose, overlayClassName, overlayStyle, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-50 grid [&>*]:min-w-0 w-full max-w-lg max-h-[90vh] overflow-y-auto outline-none translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg',
        className
      )}
      onKeyDown={(e) => {
        onKeyDown?.(e)
    ...
    >
      {children}
      {!hideClose && (
        // focus-visible (not focus): the dialog auto-focuses this button when
        // it has no other focusable content, and a mouse-opened dialog must
        // not paint a ring. Keyboard focus still shows it.
