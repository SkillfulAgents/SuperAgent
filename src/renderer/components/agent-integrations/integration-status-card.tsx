...
export function IntegrationStatusCard({ integration, connected }: {
  ...
  const state = deriveAgentIntegrationState(integration.status, connected, integration.reconnectRequired, !!integration.healthMessage)
  ...
  return (
    ...
    >
      {isOn && integration.healthMessage && <p className="text-sm text-muted-foreground">{integration.healthMessage}</p>}
    </DetailCard>
  )
}
