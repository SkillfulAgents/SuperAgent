import { AudioLines, AudioLinesOff, Music } from 'lucide-react'
 ...
 * hold-sound control says so; muting it keeps the music paused.
 */
export function VoiceModeControls({ showSpeed = true }: { showSpeed?: boolean }) {
  ...
  const label = holdSound ? (music.active ? `${music.playerName} on hold` : 'Hold sound on') : 'Hold sound off'
  const hint = !holdSound
    ? 'Hold sound muted. Click to play it while the agent works.'
    : music.active
      ? `${music.playerName} plays while the agent works. Click to keep it paused.`
      : 'Hold sound while the agent works. Click to mute.'
      ...
      <div className="flex items-center gap-0.5" data-testid="voice-mode-controls">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              {!holdSound ? <AudioLinesOff className="h-3.5 w-3.5" /> : music.active ? <Music className="h-3.5 w-3.5" /> : <AudioLines className="h-3.5 w-3.5" />}
            </button>
          </TooltipTrigger>
