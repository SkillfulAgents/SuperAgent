...
vi.mock('@renderer/hooks/use-chat-integrations', () => ({
  ...
  useDenyChatAccess: () => ({ mutate: vi.fn(), isPending: false }),
...
describe('ConversationHistorySection (inbox)', () => {
  ...
  it('shows a pending chat as Blocked with an Unblock action (no Block) for a Telegram owner', () => {
  ...
  it('clicking Unblock approves that chat with its integration + access ids', () => {
    ...
    expect(h.approve).toHaveBeenCalledWith({ integrationId: 'int-1', accessId: 'acc-chat-p' })
  ...
  it('shows Block (not Unblock) on an allowed chat and revoking it uses the right ids', () => {
    ...
    expect(h.revoke).toHaveBeenCalledWith({ integrationId: 'int-1', accessId: 'acc-chat-x' })
