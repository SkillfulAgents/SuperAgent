...
function setup() {
  ...
  const player = new SpeechPlayer({
    adapter,
    connection: { transport: 'websocket', token: 't' },
    voice: { voice: 'v' },
...
describe('SpeechPlayer', () => {
  ...
  it('a first-word offset shifts the cursor onto the whole message', () => {
    const adapter = new FakeAdapter()
    const ctx = new FakeAudioContext()
    const player = new SpeechPlayer({ adapter, connection: { transport: 'websocket', token: 't' }, voice: { voice: 'v' }, firstWordIndex: 10, createAudioContext: () => ctx as unknown as AudioContext })
    player.start()
