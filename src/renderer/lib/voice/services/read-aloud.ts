...
export const READ_ALOUD_IDLE: ReadAloudSnapshot = { activeId: null, status: 'idle', error: null, errorId: null }
...
class ReadAloudController {
  ...
  async speak(id: string, markdown: string, fromWord = 0, options: { paused?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      this.current = null
      void ctx?.close()
      this.update({ ...READ_ALOUD_IDLE, error: err instanceof Error ? err.message : 'Failed to start text-to-speech', errorId: id })
      return
    }
    if (generation !== this.generation) {
      void ctx?.close()
  ...
  stop(): void {
    ...
    if (this.snapshot.activeId !== null) {
      this.update({ ...READ_ALOUD_IDLE, error: this.snapshot.error, errorId: this.snapshot.errorId })
    }
  }
  ...
  private settleStream(error: string | null): void {
    ...
    this.player = null
    player?.stop()
    this.update({ ...READ_ALOUD_IDLE, error, errorId: error ? stream.id : null })
  }
  ...
  private settle(player: SpeechPlayer, id: string, error: string | null): void {
    ...
    this.player = null
    this.current = null
    this.update({ ...READ_ALOUD_IDLE, error, errorId: error ? id : null })
  }
