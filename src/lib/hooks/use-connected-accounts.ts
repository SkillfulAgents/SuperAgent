...
export function useInitiateConnection() {
  ...
  return useMutation<InitiateConnectionResponse, Error, { providerSlug: string }>({
    mutationFn: async ({ providerSlug }) => {
      const res = await fetch('/api/connected-accounts/initiate', {
        method: 'POST',
