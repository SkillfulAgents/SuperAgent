...
import { consumeBootstrapEnv } from '@shared/lib/container/agent-bootstrap-env-store'
...
agentBootstrap.get('/:agentSlug/env', async (c) => {
  ...
  // Single-use: a second fetch gets 404.
  const env = consumeBootstrapEnv(agentSlug)
  if (!env) {
