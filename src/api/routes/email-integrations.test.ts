...
const state = vi.hoisted(() => ({ connected: true, role: 'owner', calls: [] as { url: string; body: Record<string, unknown> | null; method: string; key: string | null }[] }))
...
vi.mock('@shared/lib/agent-integrations/agent-integration-manager', () => ({ agentIntegrationManager: { addIntegration: vi.fn(), removeIntegration: vi.fn(), pauseIntegration: vi.fn(), resumeIntegration: vi.fn(), reconcileAccess: vi.fn(), integrationCreated: vi.fn() } }))
...
const app = new Hono().route('/api/chat-integrations', router)
