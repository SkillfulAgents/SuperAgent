...
proxy.all('/:agentSlug/:accountId/:rest{.+}', async (c) => {
  ...
  // On the platform hop the token never reaches the host, so a dead token
  // (app access revoked upstream, refresh failed) cannot fail at the token
  // read. It comes back as the upstream 401 inside the hop's envelope, so
  // treat that as expiry. A local Composio key keeps today's behaviour.
  if (
    response.status === 401 &&
    account.providerName === 'composio' &&
