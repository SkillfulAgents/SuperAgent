...
function attachProviderErrorPresentations(transformed: TransformedItem[]): void {
  for (const item of transformed) {
    ...
    item.errorPresentation =
      provider.parseSpecializedError(undefined, item.content.text)
      ?? (PROVIDER_ERROR_CODES.has(item.apiError)
        ? provider.parseErrorResponse(undefined, item.content.text)
        : undefined)
  }
}
