...
import { KubernetesContainerClient } from './kubernetes-container-client'
...
export function createContainerClient(config: ContainerConfig): ContainerClient {
  ...
  if (process.env.REMOTE_RUNTIME === 'kubernetes') {
    console.log('[ContainerClient] REMOTE_RUNTIME=kubernetes, using KubernetesContainerClient')
    return new KubernetesContainerClient(config)
  }
