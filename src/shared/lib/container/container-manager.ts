...
class ContainerManager {
  ...
  /** Agents still running on a container env that a setting change replaced. */
  ...
  updateCachedStatus(agentId: string, status: 'running' | 'stopped', port: number | null): void {
    ...
    // A container that stops starts fresh next time, so it is no longer stale.
    if (status === 'stopped') this.staleAgents.delete(agentId)
  }
  ...
  // ---------------------------------------------------------------------------
  // Stale agents: containers still running on an env that a setting change
  // replaced. A stop clears one, since the next start rebuilds the env.
  // ---------------------------------------------------------------------------

  /** Record every running agent as stale. */
  markAgentsStale(): void {
    for (const slug of this.getRunningAgentIds()) this.staleAgents.add(slug)
  }

  isAgentStale(slug: string): boolean {
    return this.staleAgents.has(slug)
  }
