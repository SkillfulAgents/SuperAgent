...
describe('containerManager stale agents', () => {
  ...
  it('markAgentsStale marks every running agent; a stop or a removal clears one', () => {
    running('a', 'b')
    containerManager.markAgentsStale()
    running('c') // started after the change: fresh until the next change
    expect(['a', 'b', 'c'].map((s) => containerManager.isAgentStale(s))).toEqual([true, true, false])
    containerManager.markAgentsStale()
    expect(['a', 'b', 'c'].map((s) => containerManager.isAgentStale(s))).toEqual([true, true, true])

    containerManager.updateCachedStatus('a', 'stopped', null)
    containerManager.removeClient('b')
    expect(['a', 'b', 'c'].map((s) => containerManager.isAgentStale(s))).toEqual([false, false, true])
  })
})
