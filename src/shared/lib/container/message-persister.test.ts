...
describe('MessagePersister', () => {
  ...
  describe('context window fallback from catalog (non-Claude result events)', () => {
    ...
    it('uses the catalog contextWindow when the SDK omits it (glm via platform)', () => {
      ...
      mockClient._sendMessage({
        type: 'result',
        subtype: 'success',
        modelUsage: { 'glm-5.2': { inputTokens: 100, outputTokens: 10 } },
      })
    ...
    it('prefers the catalog window over the SDK default for a listed non-Claude model', () => {
      ...
      // The SDK reports a generic 200K default for glm; the catalog (1M) wins.
      mockClient._sendMessage({
        type: 'result',
        subtype: 'success',
        modelUsage: { 'glm-5.2': { contextWindow: 200_000 } },
      })
