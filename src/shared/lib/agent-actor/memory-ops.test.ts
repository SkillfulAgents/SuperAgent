...
import { AGENT_MEMORY_DIR as root, MAX_MEMORY_BYTES } from './memory-schema'
...
describe('agent memories through FileOps', () => {
  it('returns an empty list without creating a missing memory directory', async () => {
    ...
    const memories = createMemoryOps(files)
  ...
  it('reads malformed frontmatter without losing any text', async () => {
    ...
    expect(await memories.read('broken.md')).toMatchObject({ title: 'broken.md', content, body: content })
  })
  ...
  it('saves full Markdown and leaves the index untouched', async () => {
    ...
    const doc = await memories.read('style.md')
    const updated = original.replace('short paragraphs', 'brief explanations')
    const saved = await memories.save('style.md', updated, doc.revision)
    expect(saved.content).toBe(updated)
    expect(saved.revision).not.toBe(doc.revision)
    expect((await memories.read('style.md')).content).toBe(updated)
    expect((await memories.read('MEMORY.md')).content).toBe('index')
  })
  ...
  it('rejects stale saves and preserves the agent change', async () => {
    ...
    await expect(memories.save('style.md', 'user draft', doc.revision)).rejects.toMatchObject({ status: 409 })
    expect((await memories.read('style.md')).content).toBe('Agent changed this')
  })
  ...
  it('serializes competing API saves so only one stale revision can win', async () => {
    ...
    const results = await Promise.allSettled([
      memories.save('style.md', original + 'First', doc.revision),
      memories.save('style.md', original + 'Second', doc.revision),
    ])
    expect(results.map(result => result.status)).toEqual(['fulfilled', 'rejected'])
    expect((await memories.read('style.md')).content).toBe(original + 'First')
  })
  ...
  it.each(['../secrets.md', '/absolute.md', 'nested/../../escape.md', 'nested/../style.md', 'a\\b.md', 'a\0.md', 'a.txt'])('rejects invalid path %s before I/O', async relative => {
    ...
    await expect(memories.read(relative)).rejects.toMatchObject({ status: 400 })
  ...
  it('rejects symlink redirects through the actor resolver', async () => {
    ...
    await expect(memories.read('link.md')).rejects.toMatchObject({ status: 400 })
    await expect(memories.list()).rejects.toMatchObject({ status: 400 })
  ...
  it('does not recreate a deleted memory on save', async () => {
    ...
    await expect(memories.save('style.md', 'draft', doc.revision)).rejects.toMatchObject({ status: 404 })
  ...
  it('lists oversized files but refuses to read or overwrite them', async () => {
    ...
    expect(await memories.list()).toEqual([expect.objectContaining({ path: 'big.md' })])
    await expect(memories.read('big.md')).rejects.toMatchObject({ status: 413 })
    await expect(memories.save('big.md', 'é'.repeat(MAX_MEMORY_BYTES), 'unused')).rejects.toMatchObject({ status: 413 })
  })
  describe('frontmatter validation on save', () => {
    it.each([
      ...
      const memories = createMemoryOps(files)
      await files.putDoc(`${root}/style.md`, original)
      const current = await memories.read('style.md')
      const write = vi.spyOn(files, 'putDoc')
      await expect(memories.save('style.md', content, current.revision)).rejects.toMatchObject({ status: 422, message: expect.stringContaining(message) })
      expect(write).not.toHaveBeenCalled()
      expect((await memories.read('style.md')).content).toBe(original)
    })
    ...
    it('lets users repair existing malformed files and retry after validation failure', async () => {
      ...
      const current = await memories.read('broken.md')
      await expect(memories.save('broken.md', 'still broken', current.revision)).rejects.toMatchObject({ status: 422 })
      expect((await memories.save('broken.md', original, current.revision)).content).toBe(original)
    })
    ...
    it('exempts only the root memory index', async () => {
      ...
      for (const relative of ['MEMORY.md', 'nested/MEMORY.md']) {
        ...
        const current = await memories.read(relative)
        const save = memories.save(relative, '- [Style](style.md)', current.revision)
