...
import { extractErrorMessage, inferErrorStatus, type ProviderErrorPresentation } from './error-presentation'
...
export class ClaudeSubscriptionLlmProvider extends BaseLlmProvider {
  ...
  protected override parseErrorResponseOverride(status: number | undefined, body: unknown, apiErrorCode?: string): ProviderErrorPresentation | null {
    ...
    const expiredToken = /\btoken\s+(?:(?:has been|has|is|was)\s+)?(?:expired|revoked)\b|\b(?:expired|revoked)\s+(?:(?:oauth|access|refresh)\s+)?token\b|\bauthentication_error\b/i.test(message)
    if (apiErrorCode === 'authentication_failed' || (status ?? inferErrorStatus(message)) === 401 || authenticationError || expiredToken) {
