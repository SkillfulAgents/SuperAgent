...
describe('x-agent-policy-service (SUP-223: duplicate null-target global policies)', () => {
  ...
  it('Case 1: dedupes duplicate global list policies (last-write-wins)', async () => {
    ...
    const globalList = (await listPoliciesForCaller('alice')).filter(
    ...
    expect((await evaluate('alice', 'list', null))).toBe('block')
  })
  ...
  it('Case 2: dedupes duplicate global read policies (last-write-wins)', async () => {
    ...
    const globalRead = (await listPoliciesForCaller('alice')).filter(
    ...
    expect((await evaluate('alice', 'read', 'any-target'))).toBe('block')
  })
  ...
  it('keeps a deduped global alongside untouched specific-target rows', async () => {
    ...
    expect((await evaluate('alice', 'list', null))).toBe('block')
    expect((await evaluate('alice', 'read', 'bob'))).toBe('allow')
  })
  ...
  it('distinct globals across operations are all kept', async () => {
    ...
    expect((await listPoliciesForCaller('alice'))).toHaveLength(3)
    expect((await evaluate('alice', 'list', null))).toBe('allow')
    expect((await evaluate('alice', 'read', 'x'))).toBe('block')
    expect((await evaluate('alice', 'invoke', 'x'))).toBe('allow')
  })
})
