...
describe('Linear scoped task tools', () => {
  ...
  it('reconciles an already-created native response instead of double-posting a comment', async () => {
    const fetchMock = vi.fn(async () => Response.json({ data: { agentActivity: { id: 'publication', agentSession: { id: 'native-session' } } } }))
    vi.stubGlobal('fetch', fetchMock)
    const tasks = new LinearTasks(new LinearClient(undefined, 'token'))
    expect(await tasks.publish({ id: 'event', taskId: 'issue', interactionId: 'native-session', kind: 'invocation', timestamp: '', text: '', payload: {}, replyTarget: { agentSessionId: 'native-session' } },
