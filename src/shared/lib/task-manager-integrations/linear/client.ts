...
async function tokenFor(id: string): Promise<LinearTokens> {
  ...
  const refresh = exchangeLinearToken({ grant_type: 'refresh_token', refresh_token: oldRefresh,
    ...
    if (error instanceof LinearAuthorizationError) {
      updateLinearConfig(id, latest => latest.tokens?.refreshToken === oldRefresh && !latest.authorizationPending ? { ...latest, tokens: undefined, authorizationError: 'Linear access expired or was revoked. Reconnect this account.' } : latest)
...
export class LinearClient {
  ...
  async authorization(): Promise<{ accessToken: string; expiresAt: number }> {
    if (this.integrationId) return tokenFor(this.integrationId)
  ...
  async request<T>(query: string, variables: Record<string, unknown>, schema: z.ZodType<T>, signal?: AbortSignal): Promise<T> {
    ...
    if (response.status === 401 && this.integrationId) {
      updateLinearConfig(this.integrationId, latest => latest.tokens?.accessToken === token && !latest.authorizationPending ? { ...latest, tokens: undefined, authorizationError: 'Linear access expired or was revoked. Reconnect this account.' } : latest)
