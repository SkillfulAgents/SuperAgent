...
const refreshes = new Map<string, Promise<LinearTokens>>()
async function tokenFor(id: string): Promise<LinearTokens> {
  const config = getLinearConfig(id)
  if (!config.tokens || !config.clientId || !config.clientSecret) throw new Error('Linear is not authorized')
  if (config.tokens.expiresAt > Date.now() + 60000) return config.tokens
  ...
  const refresh = exchangeLinearToken({ grant_type: 'refresh_token', refresh_token: oldRefresh,
    client_id: config.clientId, client_secret: config.clientSecret }).then(tokens => {
    updateLinearConfig(id, latest => {
      if (latest.tokens?.refreshToken !== oldRefresh) throw new Error('Linear authorization changed during renewal')
      return { ...latest, tokens }
    })
    return tokens
  }).catch(error => {
    if (error instanceof LinearAuthorizationError) {
      updateLinearConfig(id, latest => latest.tokens?.refreshToken === oldRefresh ? { ...latest, tokens: undefined } : latest)
      if (!getLinearConfig(id).tokens) updateChatIntegrationStatus(id, 'disconnected', error.message)
    }
    throw error
...
export class LinearClient {
  ...
  async request<T>(query: string, variables: Record<string, unknown>, schema: z.ZodType<T>): Promise<T> {
    const token = this.integrationId ? (await tokenFor(this.integrationId)).accessToken : this.accessToken
    ...
    if (response.status === 401 && this.integrationId) {
      updateLinearConfig(this.integrationId, latest => latest.tokens?.accessToken === token ? { ...latest, tokens: undefined } : latest)
    ...
    const result = z.object({ data: z.unknown().optional(), errors: z.array(z.unknown()).optional() }).parse(await response.json())
