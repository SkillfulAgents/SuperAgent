...
export class LinearTasks {
  ...
  async publish(event: TaskEvent, publication: TaskPublication, assertActive: () => void = () => {}): Promise<string> {
    ...
    const agentSessionId = event.replyTarget.agentSessionId
    ...
    if (agentSessionId) {
      try {
        const existing = await client.request(`query($id:String!){agentActivity(id:$id){id agentSession{id}}}`, { id: publication.id },
          z.object({ agentActivity: z.object({ id: z.string(), agentSession: z.object({ id: z.string() }) }) }))
        if (existing.agentActivity.agentSession.id !== agentSessionId) throw new Error('Activity belongs to another session')
        return existing.agentActivity.id
      } catch { /* New IDs are normally not found. The create still uses this ID. */ }
      const result = await client.request(`mutation($input:AgentActivityCreateInput!){agentActivityCreate(input:$input){success agentActivity{id}}}`, {
        input: { id: publication.id, agentSessionId, content: { type: publication.kind, body: publication.body } },
      }, z.object({ agentActivityCreate: z.object({ success: z.boolean(), agentActivity: z.object({ id: z.string() }).nullable() }) }))
      if (!result.agentActivityCreate.success || !result.agentActivityCreate.agentActivity) throw new Error('Linear did not create the activity')
    ...
    } catch { /* See native activity reconciliation above. */ }
    const result = await client.request(`mutation($input:CommentCreateInput!){commentCreate(input:$input){success comment{id}}}`, {
      input: { id: publication.id, issueId: event.taskId, body: publication.body, ...(event.replyTarget.commentId ? { parentId: event.replyTarget.commentId } : {}) },
