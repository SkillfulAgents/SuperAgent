...
export function linearAppCreationUrl(name: string, config: Pick<LinearConfig, 'redirectUri' | 'webhookUrl'>): string {
  ...
  const params: Record<string, string> = {
    ...
    'developer.name': 'Gamut', 'webhook.enabled': 'true', 'webhook.url': config.webhookUrl,
  }
  for (const [key, value] of Object.entries(params)) url.searchParams.set(key, value)
  for (const type of ['AgentSessionEvent', 'Issue', 'Comment', 'Attachment', 'PermissionChange', 'OAuthAuthorization']) {
    url.searchParams.append('webhook.resourceTypes', type)
  }
  return url.toString()
}
