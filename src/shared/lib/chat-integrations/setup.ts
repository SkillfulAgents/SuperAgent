...
import { inputSchema, telegramResultSchema, slackResultSchema, tokenResultSchema } from './setup-schema'
  ...
  return {
    ...
    async prepare(input) {
      ...
      if (provider === 'imessage' && config.code && !config.token) {
        ...
        if (response.status === 401) throw new IntegrationSetupError('Invalid or expired code', 401)
    ...
    async testCredentials(input) {
      ...
      if (provider === 'telegram') {
        const { botToken } = z.object({ botToken: z.string().min(1, 'Missing botToken') }).parse(config)
      ...
      if (provider === 'slack') {
        const { botToken, appToken } = z.object({ botToken: z.string().min(1, 'Missing botToken'), appToken: z.string().min(1, 'Missing appToken (app-level token for Socket Mode)') }).parse(config)
      ...
      // Verification codes are consumed only by prepare(), never by a preflight check.
      if (config.code && !config.token) imessageSetupSchema.parse(config)
      else schema.parse(config)
      return { valid: true, phoneNumber: config.phoneNumber }
    },
  }
}
