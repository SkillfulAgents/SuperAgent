  ...
  async run(db) {
    ...
    for (const table of [scheduledTasks, webhookTriggers, chatIntegrations]) {
      const rows = await db.select({ model: table.model }).from(table)
        .where(and(isNull(table.connectionId), isNotNull(table.model))).all()
    ...
    for (const table of [scheduledTasks, webhookTriggers, chatIntegrations]) {
      for (const model of legacyModels) {
        await db.update(table).set(selection(model, 'agent'))
          .where(and(isNull(table.connectionId), eq(table.model, model))).run()
      }
    }
    mutateSettings((s) => {
      s.llmLegacyConnectionId ??= imported.id
