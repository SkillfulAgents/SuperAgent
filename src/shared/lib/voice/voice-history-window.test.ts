...
describe('windowVoiceHistory', () => {
  it('returns short histories intact and oldest-first', () => {
    ...
    expect(windowVoiceHistory(history)).toEqual(history.map((m) => ({ ...m, content: m.content.trim() })))
  })

  it('drops empty and whitespace-only turns', () => {
    const out = windowVoiceHistory([{ role: 'user', content: 'Research this' }, { role: 'assistant', content: '  ' }, { role: 'assistant', content: 'Found it' }])
    expect(out.map((m) => m.content)).toEqual(['Research this', 'Found it'])
  })

  it('keeps the newest turns when the token budget runs out', () => {
    const history = Array.from({ length: 60 }, (_, i) => turn(i, 60))
    const out = windowVoiceHistory(history, { ...VOICE_HISTORY_LIMITS, tokenBudget: 800 })
    expect(out.length).toBeGreaterThan(0)
    expect(out.length).toBeLessThan(history.length)
    expect(out.at(-1)?.content).toBe(history.at(-1)?.content.trim())
    const spent = out.reduce((sum, m) => sum + estimateTokenCount(m.content) + VOICE_HISTORY_LIMITS.perMessageOverheadTokens, 0)
    expect(spent).toBeLessThanOrEqual(800)
  })

  it('never exceeds the message cap', () => {
    const out = windowVoiceHistory(Array.from({ length: 200 }, (_, i) => turn(i, 1)))
    expect(out).toHaveLength(VOICE_HISTORY_MAX_MESSAGES)
    expect(out.at(-1)?.content).toBe(turn(199, 1).content.trim())
  })

  it('clips a giant pasted turn to its head instead of losing the whole window', () => {
    const giant = { role: 'user' as const, content: 'A'.repeat(3_000_000) }
    const out = windowVoiceHistory([turn(0), turn(1), giant])
  ...
  it.each(['<|endoftext|>', '<|endofprompt|>', '<|fim_prefix|>'])('counts chat text quoting the %s marker as ordinary text', (marker) => {
    ...
    expect(windowVoiceHistory(history)).toEqual(history)
    expect(windowVoiceHistory(history, { ...VOICE_HISTORY_LIMITS, tokenBudget: 1 })).toEqual([])
  })

  it('stays under the GPT-Live 8,192-token input cap with a full window of dense text', () => {
    const dense: VoiceHistory = Array.from({ length: 200 }, (_, i) => ({ role: role(i), content: '中文'.repeat(1000) }))
    const out = windowVoiceHistory(dense)
    const spent = out.reduce((sum, m) => sum + estimateTokenCount(m.content) + VOICE_HISTORY_LIMITS.perMessageOverheadTokens, 0)
    expect(spent).toBeLessThanOrEqual(VOICE_HISTORY_LIMITS.tokenBudget)
