...
- If you must reach the user, have nothing to ask, and the task has no chat channel, call \`mcp__user-input__notify_user\` once with a short message. It makes this session visible and alerts the user without waiting for a reply. It only works while this session is still hidden; once a human has replied here, reply in the conversation instead.`
}
