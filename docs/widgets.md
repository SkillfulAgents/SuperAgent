...
- `WidgetCard` (`src/renderer/components/widgets/widget-card.tsx`) — the snapshot in an `<iframe sandbox="">` (no scripts, no same-origin), URL keyed on `htmlHash`, `data-theme` from the app's resolved theme, spinner while refreshing, error badge on `lastError`, hover refresh button, "Open app" and a link to the dashboard when `hasDashboard`.
