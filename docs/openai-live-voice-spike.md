...
OpenAI also provides read-aloud using `gpt-4o-mini-tts`, with Marin as the default and a selectable voice catalogue. `/api/voice/tts-session` returns transport-neutral initialization: Deepgram receives an ephemeral WebSocket token; OpenAI receives an HTTP connection descriptor without credentials. The legacy `/tts-token` endpoint remains available for token-based providers.
