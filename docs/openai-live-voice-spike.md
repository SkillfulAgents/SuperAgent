...
- Client-delegation and transcript mapping. A delegation event contains metadata, not an agent request. OpenAI Live decides *when* to hand off; the adapter decides *what* is sent. It coalesces transcript fragments and asks the configured summarizer to rewrite the user's unsent words into one clear message (fixing speech-to-text, applying corrections, resolving references from the labeled transcript) and to pick `interrupt` or `queue`. The summarizer never decides whether to send, never asks its own questions, and never authors content from voice-assistant lines; a rewrite that repeats the voice assistant's words falls back to the user's own. Additional user fragments invalidate in-flight mappings. Duplicate delegation IDs are ignored.
...
- Transcript coalescing is a heuristic, not an authoritative end-of-turn signal. An incomplete request is sent as is; the backend agent has the context to ask. Evaluate delayed/overlapping transcript delivery with real audio before rollout.
