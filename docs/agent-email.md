...
The integration framework owns durable provider state through `agent-integrations/state-store.ts`. Providers supply their value schemas and scoped keys; only the shared store accesses the `integration_state` table. Email uses it for poll cursors, thread admission, held-mail review, upload/send intents, pending reply compositions and receipts. Slack uses the same API for bot thread participation. Generic availability metadata supports bounded retry selection without inspecting provider payloads.

Migration `0049_shared_integration_state.sql` moves existing email and Slack state into the shared store, preserving pending drafts, retry times and thread participation, then drops `email_integration_state` and `slack_thread_state`. Earlier migrations remain in history so existing preview installations can upgrade. The framework owns reset and cascade deletion. State uses awaited conditional SQL compatible with SQLite and libSQL/D1. Email and gateway retention remain indefinite.
