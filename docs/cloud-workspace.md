...
- **Documents served through the proxy don't inherit its prefix.** A dashboard
  pane or popout loads the cloud site when the desktop cookie jar has a session
  cookie for that origin. Otherwise it stays on the keyed door URL. Other
  headerless calls (SSE, screenshots, downloads) still use the prefix. The
...
The window is built in main. It uses the same cookie jar as the main window.

1. **The document URL.** Callers pass the door (`activeApiTarget().baseUrl`).
   When the jar has a session cookie, the window loads the cloud site and
   `/view`. When it does not, it keeps the door URL.
2. **No second jar.** A per-window partition and request rewrite used to put
   `/api/` calls back through the prefix. That path is gone. Files resolve on
   the origin that served the document.
3. **Identity and lifetime.** The dedup key includes the loaded base URL — two
   deployments can hold an agent of the same slug — and `applyPreferredApiTarget`
   closes all popouts on a switch.
...
an explicit cloud flag, not "the URL has a proxy prefix".
