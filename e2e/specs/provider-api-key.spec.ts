...
async function addForm(page: Page) {
  ...
  await page.getByTestId('llm-provider-picker').getByRole('button', { name: /^Anthropic/ }).click()
  await expect(page.getByLabel('API key', { exact: true })).toBeVisible()
}

async function editConnection(page: Page, name: string) {
...
test.describe('Provider connection lifecycle', () => {
  ...
  test('draft validation keeps a rejected key editable and saves nothing', async ({
    ...
    await addForm(page)
    await page.getByLabel('Connection name', { exact: true }).fill('Rejected account')
  ...
  test('adds two accounts without changing the default, switches explicitly, and protects the root', async ({
    ...
    await page.getByRole('option', { name: firstName, exact: true }).click()
    ...
    const firstMenu = page.getByRole('button', { name: `Actions for ${firstName}`, exact: true })
    const deleteFirst = page.getByRole('button', { name: 'Delete', exact: true })
    await firstMenu.click()
    await expect(deleteFirst).toBeDisabled()
    // The menu explains why the delete is blocked.
    await expect(page.getByText('Change the app default before deleting this connection', { exact: true })).toBeVisible()
    await page.keyboard.press('Escape')
    await page.getByTestId('settings-model-trigger').first().click()
    await page.getByRole('combobox', { name: 'Connection' }).click()
    await page.getByRole('option', { name: secondName, exact: true }).click()
    await page.keyboard.press('Escape')
    await firstMenu.click()
  ...
  test('waits for a connection save before accepting a model choice', async ({ page, request }) => {
    ...
    try {
      ...
      await page.getByRole('option', { name: 'Delayed anthropic', exact: true }).click()
      ...
      await expect(connection).toHaveText('Delayed anthropic')
      await haiku.click()
  ...
  test('switches an existing session to another account without changing the app default', async ({
    ...
    await expect(page.getByRole('combobox', { name: 'Connection' })).toHaveText('Session account A')
    await page.getByRole('combobox', { name: 'Connection' }).click()
    await page.getByRole('option', { name: 'Session account B', exact: true }).click()
    await page.keyboard.press('Escape')
    ...
    await page.reload()
    await page.getByTestId('composer-options-trigger').click()
    await expect(page.getByRole('combobox', { name: 'Connection' })).toHaveText('Session account B')
  })
  test('edits custom and disabled models without saving built-in catalog definitions', async ({ page, request }) => {
    ...
    await page.goto('/settings/llm')
    await editConnection(page, 'Catalog account')
    await page.getByTestId('connection-advanced-trigger').click()
  ...
  test('keeps custom variables scoped to a provider and supports masked edits and removal', async ({ page, request }) => {
    await addForm(page)
    await page.getByLabel('Connection name', { exact: true }).fill('Environment account')
    await page.getByLabel('API key', { exact: true }).fill('sk-ant-e2e-placeholder')
    await page.getByTestId('connection-advanced-trigger').click()
    ...
    await page.getByRole('button', { name: 'Save', exact: true }).click()
    await editConnection(page, 'Environment account')
    await page.getByTestId('connection-advanced-trigger').click()
    const saved = await (await request.get('/api/llm-connections')).json()
    ...
    await page.getByLabel('Connection name', { exact: true }).fill('Renamed environment account')
    ...
    await editConnection(page, 'Renamed environment account')
    await page.getByTestId('connection-advanced-trigger').click()
  ...
  test('preserves custom endpoints on unrelated Generic and Bedrock edits', async ({ page, request }) => {
    for (const [provider, apiKeys, runtimeEnv] of [
      ...
      const { id } = await created.json()
      await page.goto('/settings/llm')
      await editConnection(page, `${provider} custom env`)
      await page.getByLabel('Connection name', { exact: true }).fill(`${provider} renamed`)
      const saved = page.waitForRequest(req => req.method() === 'PUT' && req.url().endsWith(`/llm-connections/${id}`))
      await page.getByRole('button', { name: 'Save', exact: true }).click()
