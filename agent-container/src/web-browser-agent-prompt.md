...
## Core Workflow
0. **Read the status line first.** `browser_open` reports where the browser actually landed (final URL, title, HTTP status), and every snapshot starts with `[page] URL · "title" · HTTP status · readyState · N refs`. A `⚠` there (site unreachable, bot-block, HTTP 4xx/5xx, raw file, still loading, still busy) is the first thing to act on: a bot-block or login wall means `request_browser_input`, not more scraping; "still loading" or "still busy: spinner, 2 requests in flight" means the page is working — `browser_wait` for the element you need, not a re-snapshot loop.
...
4. **Read the action result** — click/press/select/hover/scroll results report whether the page navigated plus an `Effect:` line: dialogs opened or closed, live-region announcements (toasts, validation errors), failed requests, typed field values, and the change in interactive elements. The harness already waited for the page to stop working (spinners, requests in flight) before reporting, up to 2s. Fill results report the field's actual committed value. Trust these; don't re-snapshot just to confirm an action worked. `Effect: no DOM change` is a real signal — the click was probably swallowed (disabled, covered, wrong target); check the element's state or `browser_wait` for what you expect rather than clicking again. `Effect: still busy after 2.0s (…)` means the page is slow, not broken — `browser_wait` for what you expect, then re-snapshot. (`browser_open` reports the landing page instead — see step 0.)
