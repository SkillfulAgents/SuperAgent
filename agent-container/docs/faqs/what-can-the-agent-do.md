...
description: Capability overview — the kinds of tasks agents handle and the platform features available.
---
...
- **Browse the web** — a real browser the user can watch live, for research, form-filling, and downloads.
...
- **Build dashboards and artifacts** — live web apps served from the agent's container for visualizing data or delivering rich output.
- **Run on a schedule or on events** — recurring/one-time scheduled tasks, and webhook triggers that start a session when something happens externally (new email, new PR, a Stripe event).
